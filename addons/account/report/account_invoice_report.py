    ...
    def _sub_select(self):
        select_str = """
                SELECT ail.id AS id,
                    ...
                    ai.partner_bank_id,
                    SUM ((invoice_type.sign_qty * ail.quantity) / COALESCE((u.factor * u2.factor), 1)) AS product_qty,
                    SUM(ail.price_subtotal_signed * invoice_type.sign) AS price_total,
                    SUM(ai.amount_total * invoice_type.sign_qty) AS amount_total,
                    SUM(ABS(ail.price_subtotal_signed)) / CASE
                            WHEN SUM(ail.quantity / COALESCE((u.factor * u2.factor), 1)) <> 0::numeric
                               THEN SUM(ail.quantity / COALESCE((u.factor * u2.factor), 1))
                               ELSE 1::numeric
